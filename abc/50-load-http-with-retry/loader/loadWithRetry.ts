...
export function loadWithRetry<S, T>(
  ...
  return source.switchMap<ILoadResult<T>>(key => {
    let sideChannel = new Subject<ILoadResult<T>>();
    let attempt = 0;
    return Observable.merge(
      ...
      Observable.defer(() => {
        ...
        .map(data => ({ status: ILoadResultStatus.Success, data })));
  });
}

function retryDelay(options: ILoadWithRetryOptions, attempt: number): Observable<any> {
  let jitter = (Math.random() - .5) * options.retryDelayMs * .5;
